...
import argparse
from fermi.square import brillouin
from fermi.square.surface import const_energy_line
from fermi.square.patches import get_patches, find_patch
...
def precompute(args):
    ...
    brlu = brillouin()
    #找到patches
    pats = get_patches(brlu, args.patches, 'square')
    ...
    #patches和费米面的图片
    lsur = const_energy_line(ltris, ladjs, 0., 'square')
    patches_visualize(pats, lsur, '{0}_surface_{1}.svg'.format(args.prefix, args.disp))
    #求出每个Rtriangle所在的patch
    lpats = [find_patch(tri.center, pats, 'square') for tri in ltris]
