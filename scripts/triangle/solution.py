...
from fermi.triangle import shift_kv, get_patches, dispersion
from fermi.triangle import get_exchange_gamma
...
def load_brillouin(args):
    ...
    lpats = patload('{0}_pats.txt'.format(args.prefix))
    #整理成数组
...
def slove_equ(args, brlu, ltris, ladjs, mpinfo, mlpats):
    ...
    uval += 0
    #初始化J
    jgam = get_exchange_gamma(1.0, mpinfo[0, :])
    uval[0, 0, 0, 0, :, :, :] += jgam
    print(uval)
...
def main():
    ...
    parser.add_argument('--prefix', type=str, default='scripts/triangle/tri',\
